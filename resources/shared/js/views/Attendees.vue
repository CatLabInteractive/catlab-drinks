            ...
            <b-col cols="12" id="attendees-page">
                ...
                <div v-if="loaded">
                    ...
                    <div class="alert alert-info">
                        <p>
                            <strong>{{ $t('About attendees:') }}</strong>
                            {{ $t('Attendees link card aliases (secret tokens) to personal information such as name and email.') }}
                            {{ $t('This allows NFC card holders to place remote orders using their alias without physically scanning their card.') }}
                            {{ $t('The alias must remain secret, as it grants access to spend credit from the linked card.') }}
                        </p>
                        <p>
                            {{ $t('To actually link a card to an alias, the organiser must check in the attendee using the Check-In module.') }}
                            {{ $t('Only after check-in is the alias linked to the NFC card.') }}
                            {{ $t('Aliases expire after 24 hours.') }}
                        </p>
                        <p class="mb-0">
                            {{ $t('Aliases can be generated by external tools such as') }} <a href="https://www.quizwitz.com" target="_blank" rel="noopener noreferrer">QuizWitz.com</a>.
                            {{ $t('You can paste tab-separated or colon-separated data directly into the spreadsheet below.') }}
                        </p>
                    </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            columns() {
                return [
                    { key: 'alias', label: this.$t('NFC Card Alias') },
