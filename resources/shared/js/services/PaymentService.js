...
export class PaymentService extends Eventable {
	...
	hasPaymentMethod() {
		...
		return this.allow_nfc_payments || this.allow_cash_payments || this.allow_voucher_payment;
    ...
    async order(order, acceptCurrentCard = true) {
		...
		// Do we have a payment method configured? If not, just leave the order unpaid.
	...
	visualiseCurrency(price) {
		// Display in european format, with 2 decimals and a comma 
		// as a decimal separator.
		return this.currencySymbol + (price.toFixed(2).replace('.', ','));
	}

	visualisePrice(price) {

		var parts = [
			this.visualiseCurrency(price)
		];

		if (this.allow_voucher_payment) {
			parts.push('(' + Math.ceil(price / this.voucher_value) + ' vouchers' + ')');
		}
